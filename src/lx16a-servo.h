...
public:
	 ...
	 * 
	 * Edited: why do all of this when you can just calibrate 
	 * manually for the arm once, and then load that every time?
	 * It's not like you're going to be swapping servos every five minutes
	 *
	 */
	bool calibrate(int32_t currentPos, int32_t currentAngleCentDegrees,int32_t min_angle_cent_deg, int32_t max_angle_cent_deg){
		...
		int32_t current;
		initialize();
		//just calibrate based on known values. The servos don't change run-to-run.
		// do{
			//pos_read();
			current=currentPos*24;
		// 	if(!isCommandOk()){
		...
		if(min_angle_in_Ticks<0||max_angle_in_Ticks>1000){
			...
			//servo limits should be set per-motor by hand to avoid damaging them
